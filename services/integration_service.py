...
def integrate_similarities(
    ...
    Args:
        text_results: テキスト類似度結果のリスト [{"id": str, "similarity": float}, ...]
    ...
    # IDをキーとした辞書に変換
    text_dict = {item["id"]: item["similarity"] for item in text_results}
    ...
    for id_value in all_ids:
        # 各類似度を取得（存在しない場合は0）
        text_similarity = text_dict.get(id_value, 0.0)
