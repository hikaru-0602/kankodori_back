...
def text_similarity_sort(
    ...
    """
    テキスト検索用：フィルタリングされたデータとクエリベクトル間のコサイン類似度を計算し、ソートする
...
def image_similarity_sort(
    ...
    """
    画像検索用：フィルタリングされたデータとクエリベクトル間のコサイン類似度を計算し、ソートする

    Args:
        filtered_data: 地名でフィルタリングされたデータリスト
        query_vector: クエリ画像のベクトル
        features: npyファイルから取得したベクトル配列
        labels: ベクトルに対応するIDリスト
